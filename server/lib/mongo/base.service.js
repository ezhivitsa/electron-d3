...
class BaseService extends BaseQueryService {
  ...
  create (entities) {
    ...
    return this._collection.insertAsync(entities)
      .then((data) => {
        entities = capitalLetterInterceptor(entities)
  ...
  update (query, updateFn, options) {
    return this.findOne(query, options)
      ...
      .then((data) => {
        let newData = capitalLetterInterceptor(data[1])
        this.emit('updated', {
          doc: newData,
          prevDoc: data[2]
        })
  ...
  getNextId (id, tenantId) {
    ...
    return this._collection.findAndModifyAsync(query, {}, { $inc: { Seq: 1 } }, {
      new: true,
      upsert: true
    })
    .then(res => {
